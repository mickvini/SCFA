...
WindEnergyCreationUnit = Class(EnergyCreationUnit) {
    ...
    OnStopBeingBuilt = function(self,builder,layer)
        ...
        if not WindEnergyMin and not WindEnergyRange then
            ...
            local mean = bp.ProductionPerSecondEnergy or 7
            local min = bp.ProductionPerSecondEnergyMin or 0.2
            local max = bp.ProductionPerSecondEnergyMax or 24
        ...
        -- Run the thread
        ------------------------------------------------------------------------
        self:ForkThread(SyncroniseThread,0.30,self.OnWeatherInterval,self)
    end,
...
TidalEnergyCreationUnit = Class(EnergyCreationUnit) {
    ...
    OnStopBeingBuilt = function(self,builder,layer)
        ...
        if not TidalEnergyMin and not TidalEnergyRange then
            ...
            --------------------------------------------------------------------
            local bp = self:GetBlueprint().Economy
            local mean = bp.ProductionPerSecondEnergy or 5
            local min = bp.ProductionPerSecondEnergyMin or 2.0
            local max = bp.ProductionPerSecondEnergyMax or 8.0
            local range = max - min
            if (min + max) / 2 ~= mean then
                local mult = mean / 25
                min = min * mult
