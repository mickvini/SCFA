...
uel0402 = Class(TWalkingLandUnit) {
    ...
    NukeExplosion = function(self)
        ...
        CreateAttachedEmitter(self, 'uel0402', Army, '/effects/emitters/destruction_explosion_concussion_ring_03_emit.bp'):ScaleEmitter(2)
        self:ForkThread(self.CreateOuterRingWaveSmokeRing)
        CreateLightParticle(self, -1, Army, 40, 9, 'glow_02', 'ramp_red_02')
        WaitSeconds( 0.25 )
        CreateLightParticle(self, -1, Army, 40, 80, 'glow_03', 'ramp_fire_06')
        WaitSeconds( 0.55 )        
        CreateLightParticle(self, -1, Army, 80, 1000, 'glow_03', 'ramp_nuke_04')
      
        # Create ground decals
        local orientation = RandomFloat( 0, 2 * math.pi )
        CreateDecal(position, orientation, 'Crater01_albedo', '', 'Albedo', 80, 80, 4800, 0, Army)
        CreateDecal(position, orientation, 'Crater01_normals', '', 'Normals', 80, 80, 4800, 0, Army)      
        CreateDecal(position, orientation, 'nuke_scorch_003_albedo', '', 'Albedo', 80, 80, 4800, 0, Army)
   
        # Knockdown force rings
        DamageRing(self, position, 0.1, 80, 1, 'Force', false)
        WaitSeconds(0.1)

        #Nuke damage and camera shake
        DamageRing(self, position, 0.1, 30, 5000, 'Force', true)
		DamageRing(self, position, 0.1, 20, 10000, 'Force', true)
		DamageRing(self, position, 0.1, 10, 25000, 'Force', true)
