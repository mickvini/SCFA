...
UnitTeleportEffect01 = Class(NullShell) {
    ...
    TeleportEffectThread = function(self)
        ...
        DamageRing(self, pos, .1, 10, 100, 'Force', false, false)
		...
		WaitSeconds(.1)
        DamageRing(self, pos, .1, 6, 100, 'Force', false, false)

		# Knockdown force rings
        WaitSeconds(0.39)
        DamageRing(self, pos, 1, 8, 50, 'Fire', false, false)
        WaitSeconds(.1)
        DamageRing(self, pos, 1, 10, 100, 'Fire', false, false)
        WaitSeconds(0.5)

        # Scorch decal and light some trees on fire
        WaitSeconds(0.3)
        DamageRing(self, pos, 5, 20, 100, 'Fire', false, false)
    end,
