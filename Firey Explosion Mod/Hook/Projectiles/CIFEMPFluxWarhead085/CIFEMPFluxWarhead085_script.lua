#****************************************************************************
#**
#**  File     :  /projectiles/CIFEMPFluxWarhead085/CIFEMPFluxWarhead085_script.lua
...
CIFEMPFluxWarhead085 = Class(NullShell) {
    ...
    CreateNuclearExplosion = function(self)
        ...
        CreateLightParticle(self, -1, self:GetArmy(), 1600, 1600, 'beam_white_01', 'ramp_red_09')
        self:ShakeCamera( 600, 24, 0, 160 )
    ...
    ForceThread = function(self)
        ...
        CreateDecal(self:GetPosition(), RandomFloat(0,2*math.pi), 'nuke_scorch_001_albedo', '', 'Albedo', 224, 224, 4000, 0, army)
        DamageRing(self, pos, 2, 7.5, 1, 'Force', true)
        WaitSeconds(0.5)
        DamageArea(self, pos, 7.5, 1, 'Force', true)
    end,
    ...
    StarCloudDispersal = function(self)
        local numProjectiles = 40
        local angle = (4*math.pi) / numProjectiles
        local angleInitial = RandomFloat( 0, angle )
        local angleVariation = angle * 2
        local projectiles = {}
    ...
    PlumeVelocityThread = function(self, plume)
        plume:SetVelocity(0,2.675 * self.PlumeVelocityScale,0)
        WaitSeconds(0.5)
        plume:SetVelocity(0,115 * self.PlumeVelocityScale,0)
        WaitSeconds(0.5)
        plume:SetVelocity(0,225 * self.PlumeVelocityScale,0)
        WaitSeconds(1.3)
        plume:SetVelocity(0,135 * self.PlumeVelocityScale,0)
        #WaitSeconds(18)
    ...
    PlumeThread = function(self, plume, scale )
        ...
        # Anim Time : 1.0 sec
        plume:SetScale(0.458 * scale,0.458 * scale,0.458 * scale)
...
TypeClass = CIFEMPFluxWarhead085
