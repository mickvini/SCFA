#****************************************************************************
#**
#**  File     :  /projectiles/CIFEMPFluxWarhead07/CIFEMPFluxWarhead07_script.lua
...
AIFQuantumWarhead08 = Class(NullShell) {
    ...
    CreateNuclearExplosion = function(self)
        ...
        CreateLightParticle(self, -1, army, 1600, 1600, 'beam_white_01', 'ramp_quantum_warhead_flash_01')
    ...
    ShakeAndBurnMe = function(self, army)
        self:ShakeCamera( 600, 12, 0, 80 )
        WaitSeconds( 0.5 )
        #CreateDecal(position, heading, textureName, type, sizeX, sizeZ, lodParam, duration, army)");
        local orientation = RandomFloat(0,2*math.pi)
        CreateDecal(self:GetPosition(), orientation, 'Crater01_albedo', '', 'Albedo', 400, 400, 9600, 0, army)
        CreateDecal(self:GetPosition(), orientation, 'Crater01_normals', '', 'Normals', 400, 400, 9600, 0, army)
        self:ShakeCamera( 840, 80, 0, 16 )
        WaitSeconds( 2 )
        self:ShakeCamera( 600, 8, 0, 120 )
    end,


    InnerCloudFlares = function(self, army)
        local numFlares = 400
        local angle = (4*math.pi) / numFlares
        ...
        for i = 0, (numFlares - 1) do
            ...
            if math.mod(i,11) == 0 then
                CreateLightParticle(self, -1, army, 104, 24, 'beam_white_01', 'ramp_quantum_warhead_flash_01')
            end
