...
AIFQuantumWarhead10 = Class(NullShell) {
    ...
    NormalEffects = {'/mods/REL Nuke Mod 1.46/effects/emitters/quantum_warheadlow_01_emit.bp',},
    ...
    CreateNuclearExplosion = function(self)
        ...
        CreateLightParticle(self, -1, army, 100, 100, 'beam_white_01', 'ramp_quantum_warhead_flash_01')
    ...
    ShakeAndBurnMe = function(self, army)
        self:ShakeCamera( 35, 3, 0, 10 )
        WaitSeconds( 0.5 )
        #CreateDecal(position, heading, textureName, type, sizeX, sizeZ, lodParam, duration, army)");
        local orientation = RandomFloat(0,2*math.pi)
        CreateDecal(self:GetPosition(), orientation, 'Crater01_albedo', '', 'Albedo', 25, 25, 600, 0, army)
        CreateDecal(self:GetPosition(), orientation, 'Crater01_normals', '', 'Normals', 25, 25, 600, 0, army)
        self:ShakeCamera( 55, 10, 0, 2 )
        WaitSeconds( 2 )
        self:ShakeCamera( 35, 1, 0, 15 )
    end,
    ...
    InnerCloudFlares = function(self, army)
        ...
        for i = 0, (numFlares - 1) do
            ...
            if math.mod(i,11) == 0 then
                CreateLightParticle(self, -1, army, 7, 1.5, 'beam_white_01', 'ramp_quantum_warhead_flash_01')
            end
    ...
    DistortionField = function( self )
        ...
        proj:SetScaleVelocity(0.0615 * scale,0615 * scale,0615 * scale)
        WaitSeconds(17.0)
        proj:SetScaleVelocity(0.005 * scale,0.005 * scale,0.005 * scale)
    end,
}

TypeClass = AIFQuantumWarhead10
