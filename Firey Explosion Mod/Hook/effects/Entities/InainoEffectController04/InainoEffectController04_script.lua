#****************************************************************************
#**
#**  File     :  \data\effects\Entities\InainoEffectController04\InainoBombEffectController04_script.lua
...
InainoEffectController02 = Class(NullShell) {
    ...
    MainBlast = function( self, army )
        ...
        CreateLightParticle(self, -1, self:GetArmy(), 1280, 112, 'flare_lens_add_03', 'ramp_white_07' )
        
        # Create our decals
        CreateDecal( self:GetPosition(), RandomFloat(0.0,48.224), 'Scorch_012_albedo', '', 'Albedo', 360, 360, 4000, 0, self:GetArmy())          
        ...
        self:ShakeCamera( 440, 80, 0, 20 )        

		WaitSeconds(0.3)
        
        # Create upward moving smoke plume
        local plume = self:CreateProjectile('/effects/entities/SIFInainoStrategicMissileEffect04/SIFInainoStrategicMissileEffect04_proj.bp', 0, 12, 0, 0, 0, 0)
        ...
        local num_projectiles = 128        
        local horizontal_angle = (2*math.pi) / num_projectiles
        local angleInitial = RandomFloat( 0, horizontal_angle )  
        local xVec, zVec
        local offsetMultiple = 240.0
        local px, pz
        ...
        for i = 0, (num_projectiles -1) do            
            ...
            proj:SetLifetime(24.0)
            proj:SetVelocity(80.0)
    ...
    CreateGroundFingers = function(self)  
        ...
        for i = 0, (num_projectiles -1) do            
            ...
            local proj = self:CreateProjectile( SIFInainoStrategicMissileEffect02, px, 4.0, pz, xVec, 0.0, zVec )
            proj:SetVelocity(30)    
        end        
    end,
    ...
    CreateInitialFingers = function(self)
        ...
        for i = 0, (num_projectiles -1) do            
            ...
            local proj = self:CreateProjectile( SIFInainoStrategicMissileEffect01, px, 1.0, pz, -xVec, 1.0, -zVec )
            proj:SetVelocity(160)
            proj:SetBallisticAcceleration(-2.5)            
        end        
    end,
    
    CreatePlumes = function(self)
        # Create fireball plumes to accentuate the explosive detonation
        local num_projectiles = 28        
        local horizontal_angle = (2*math.pi) / num_projectiles
        local angleInitial = RandomFloat( 0, horizontal_angle )  
...
TypeClass = InainoEffectController04
