#****************************************************************************
#**
#**  File     :  /effects/Entities/UEFNukeEffectController04/UEFNukeEffectController04_script.lua
...
UEFNukeEffectController04 = Class(NullShell) {
    ...
    EffectThread = function(self)
        ...
        CreateLightParticle(self, -1, army, 280, 16, 'glow_02', 'ramp_red_02')
        WaitSeconds(0.25)
        CreateLightParticle(self, -1, army, 640, 80, 'glow_03', 'ramp_fire_06')
      ...
      CreateEmitterAtEntity(self, army, v )
        ...
        CreateLightParticle(self, -1, army, 2400, 2000, 'glow_03', 'ramp_nuke_04')
        
        # Create ground decals
        local orientation = RandomFloat(0,2*math.pi)
        CreateDecal(position, orientation, 'Crater01_albedo', '', 'Albedo', 400, 400, 9600, 0, army)
        CreateDecal(position, orientation, 'Crater01_normals', '', 'Normals', 400, 400, 9600, 0, army)       
        CreateDecal(position, orientation, 'nuke_scorch_003_albedo', '', 'Albedo', 480, 480, 9600, 0, army)    

    # Knockdown force rings
        DamageRing(self, position, 0.1, 90, 1, 'Force', true)
        WaitSeconds(0.1)
        DamageRing(self, position, 0.1, 90, 1, 'Force', true)
    ...
    CreateOuterRingWaveSmokeRing = function(self)
        ...
        for k, v in projectiles do
            v:SetAcceleration(-0.225)
        end         
    end,      
    
    CreateFlavorPlumes = function(self)
        local numProjectiles = 64
    ...
    local sides = 80
    local angle = (2*math.pi) / sides
    local inner_lower_limit = 8
        local outer_lower_limit = 8
        local outer_upper_limit = 8
    
    local inner_lower_height = 8
    local inner_upper_height = 24
    local outer_lower_height = 8
    local outer_upper_height = 24
...
TypeClass = UEFNukeEffectController04
