...
function CreateDefaultHitExplosion( obj, scale )
    if obj and not obj:BeenDestroyed() then
		...
		if scale < 1 then
			CreateEffects( obj, army, EffectTemplate.FireCloudSml01 )   
		else
...
function CreateDefaultHitExplosionAtBone( obj, boneName, scale )
	...
	if layer == 'Sub' then
		if scale == -1 then
			CreateBoneEffects( obj, boneName, army, EffectTemplate.ExplosionSmallUnderWater )
		end
		CreateBoneEffects( obj, boneName, army, EffectTemplate.ExplosionStructuralUnderWater )
		else			
			if scale < 1.1 then
				CreateBoneEffects( obj, boneName, army, EffectTemplate.ExplosionStructural )
			else
...
function firespray( obj, scale )
	##LOG( 'So firespray somehow does start' )
	local army = obj.Spec.Army
	local layer = obj:GetCurrentLayer()
	if layer == 'Air' then
	local velocityx, velocityy, velocityz =  obj:GetVelocity()
	local pos = obj:GetPosition()
		local numBones = obj:GetBoneCount() - 1
			chance = 1/math.pow(numBones, 1/3)
			##LOG('------++--', chance)
			for boner = 0, numBones do 
				if chance > utilities.GetRandomFloat(0, 1) then
				##LOG( 'So where are the f*cking plumes?' )
				bonerpos = obj:GetPosition(boner)
				##LOG('pos -- ', pos)
				velocityx = velocityx + utilities.GetRandomFloat(-0.3, 0.3)
				velocityy = velocityy + utilities.GetRandomFloat(-0.3, 0.3)
				velocityz = velocityz + utilities.GetRandomFloat(-0.3, 0.3)
				local speed = math.sqrt((velocityx*velocityx)+(velocityy*velocityy))
				proj = obj:CreateProjectile('/effects/entities/DestructionFirePlume01/DestructionFirePlume01_proj.bp', bonerpos.x - pos.x, bonerpos.y - pos.y, bonerpos.z - pos.z, velocityx + (bonerpos.x - pos.x)/3, velocityy + (bonerpos.y - pos.y)/6, velocityz + (bonerpos.z - pos.z)/3)
				proj:SetBallisticAcceleration(utilities.GetRandomFloat(-2, -1)):SetVelocity(utilities.GetRandomFloat(speed*4, speed*6)):SetCollision(true)
	
				local emitter = CreateEmitterOnEntity(proj, 1, '/mods/Firey Explosion Mod/hook/effects/Emitters/firedebris.bp')
				local emitter = CreateEmitterOnEntity(proj, 1, '/mods/Firey Explosion Mod/hook/effects/Emitters/firedebris_refract.bp')
				local emitter = CreateEmitterOnEntity(proj, 1, '/mods/Firey Explosion Mod/hook/effects/Emitters/firedebris_smoke.bp')
				local emitter = CreateEmitterOnEntity(proj, 1, '/mods/Firey Explosion Mod/hook/effects/Emitters/firedebris_light.bp')
...
function _CreateScalableUnitExplosion( obj )
    ...
    #LOG(scale)
    if layer == 'Land' then 
	    if scale < 0.4 then   ## Small units
             BaseEffectTable = EffectTemplate.ExplosionTiny   
		elseif scale >= 0.4 and scale < 1.1 then
             BaseEffectTable = EffectTemplate.ExplosionSmall
		elseif scale > 3.75 then ## Large units
				local Sides = 12
				local Angle = (2*math.pi) / Sides
					for i = 0, (Sides-1) do
						local velocityx = math.sin(i*Angle)
						local velocityz = math.cos(i*Angle)

						local proj = obj:CreateProjectile('/effects/entities/DestructionFirePlume01/DestructionFirePlume01_proj.bp', velocityx, 0, velocityz, velocityx, 0, velocityz)
						proj:SetBallisticAcceleration(0):SetVelocity(utilities.GetRandomFloat(6, 8)):SetCollision(false)
						local emitter = CreateEmitterOnEntity(proj, 1, '/mods/Firey Explosion Mod/hook/effects/Emitters/dirt_slice.bp')
						local emitter = CreateEmitterOnEntity(proj, 1, '/mods/Firey Explosion Mod/hook/effects/Emitters/dust_slice.bp')
				end        
			BaseEffectTable = EffectTemplate.ExplosionLarge
			ShakeTimeModifier = 2.0  ## 1.0
			ShakeMaxMul = 0.5 ## 0.25
		else                  ## Medium units
	...
	if layer == 'Water' then 
		...
		elseif scale > 3.75 then ## Large units 
			BaseEffectTable = EffectTemplate.ExplosionLargeWater
			ShakeTimeModifier = 1.0
			ShakeMaxMul = 0.25
		else                  ## Medium units
